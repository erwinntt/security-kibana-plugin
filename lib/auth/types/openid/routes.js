...
module.exports = async function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, openIdEndPoints) {
    ...
    server.ext('onPreResponse', function(request, h) {
        ...
        if (request.response.isBoom && request.path.indexOf(`${APP_ROOT}${routesPath}login`) > -1 && request.response.output.statusCode === 500) {
            server.log(['error', 'security'], `Error occured while obtaining token: ${error}`);
    ...
    try {
        ...
        */
        server.route({
            ...
            handler: async(request, h) => {
                if (!request.auth.isAuthenticated) {
                    server.log(['error', 'security'], `Error occured. Not authenticated.`);
                    return h.redirect(basePath + '/customerror?type=authError');
                }
                ...
                try {
                    ...
                    if (error instanceof AuthenticationError) {
                        server.log(['error', 'security'], `Authentication error occured in login page: ${error}`);
                        return h.redirect(basePath + '/customerror?type=authError');
                    } else if (error instanceof MissingTenantError) {
                        return h.redirect(basePath + '/customerror?type=missingTenant');
                    } else {
                        server.log(['error', 'security'], `Unexpected error occured in login page: ${error}`);
                        return h.redirect(basePath + '/customerror?type=authError');
                    }
        ...
        server.log(['error', 'security'], `Unhandled error occured in login page: ${error}`);
